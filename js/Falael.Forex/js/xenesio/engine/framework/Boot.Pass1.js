...
(function ()
{
	...
	if (Boot.pass1Complete)
	{
		return;
	}
	Boot.pass1Complete = true;

	////////////////////////////////////////////
	//	global - new functions
	if (!("log" in window)) window.log = console.log.bind(console);

	////////////////////////////////////////////
	//	global - js extensions

	window.getQueryStringField = function (name)
	{
	...
	window.stripHtmlFileFromPath = function (path)
	{
		var splitResult = path.split('/');
		var parts = [];
		for (var length = splitResult.length, i = 0; i < length; ++i)
		{
		...
		if (lastPart.indexOf(fileExtension) != lastPart.length - fileExtension.length)
		{
			return "/" + parts.join('/') + "/";
		}
		parts.pop();
	...
	window.copyAppConfigFromMetaTags = function (targetObject)
	{
		document.querySelectorAll("meta[name*='appConfig.']").forEach(function (tag) { targetObject[tag.getAttribute("name").replace("appConfig.", "")] = tag.getAttribute("content"); });
	}

	//	https://stackoverflow.com/questions/105034/create-guid-uuid-in-javascript
	window.newUUID = function ()
	{
	...
	window.newLUID = function ()
	{
		var result = window.newLUID._counter;
		++window.newLUID._counter;
		if (window.newLUID._counter == Number.MAX_SAFE_INTEGER) window.newLUID._counter = Number.MIN_SAFE_INTEGER;
		else if (window.newLUID._counter == 0) throw "Counter overflow.";
		return result;
	}
	window.newLUID._counter = 0;
	...
	{
		var element = window.measureText.__canvas;
		if (!element)
		{
			element = document.createElement("canvas");
			window.measureText.__canvas = element;
		}
		var context = element.getContext("2d");
		context.font = font;
		return {
			width: context.measureText(text).width,
			height: parseInt(context.font) * 1.2,
		};
	};

	//	https://stackoverflow.com/questions/5598743/finding-elements-position-relative-to-the-document
	window.getBoundingDocumentRect = function (element)
	{
		var clientRect = element.getBoundingClientRect();
		return {
			x: Math.round(clientRect.left + document.body.scrollLeft),
            y: Math.round(clientRect.top + document.body.scrollTop),
	...
	{
		var _toFormattedString = function (useLocale, args)
		{
			var result = '';
			var format = args[0];
	...
	String.formatMoney = function (value, decimalPlacesArg)
	{
		var decimalPlaces = (decimalPlacesArg === void (0) ? 2 : decimalPlacesArg);
		...
		if (decimalPlaces)
		{
			...
			else if (result.length - foundAt - 1 < decimalPlaces)
			{
				result = result.substr(0, foundAt + 1) + result.substr(foundAt + 1).padEnd(decimalPlaces, '0');
			}
		}
		return result;
	}

	//http://home.nerbonne.org/A-vs-An, Apache 2.0 license
	//https://github.com/EamonNerbonne/a-vs-an/blob/master/A-vs-An/AvsAn-JsDemo/AvsAn.js
	//by Eamon Nerbonne (from http://home.nerbonne.org/A-vs-An), Apache 2.0 license
	String.prototype.getUnspecificArticle = function ()
	{
		var _avsAn = (function ()
		{
			...
			return {
				raw: root,
				//Usage example: _avsAn.query("example ") 
		...
		return _avsAn.query(this + " ").article;
	}
	...
	Math.roundWithPrecision = function (value, roundingPrecision)
	{
		var factor = Math.pow(10, roundingPrecision);
	...
	Object.aggregate = function (obj, data)
	{
		for (var key in data)
		{
			obj[key] = data[key];
	...
	Object.flatten = function (src, result, path)
	{
		if (src === void 0)
		{
			return;
		...
		if (Object.prototype.toString.apply(src) === '[object Array]')
		{
			...
			{
				Object.flatten(src[i], result, path + "[" + i + "]");
			}
			return result;
		}
		...
		if (typeof src === "object")
		{
			for (var key in src)
			{
				...
				Object.flatten(src[key], result, path != "" ? path + "." + bareKey : bareKey);
			}
	...
	Array.inflate = function(values)
	{
		var STATE_INPUT = 1;
		var STATE_IDENTIFIER = 2;
		var STATE_INDEX = 3;
		var STATE_OPERATOR = 4;
		...
		for (var path in values)
		{
			...
			for (var length = path.length, i = 0; i < length;) 
			{
				var c = path[i];
				switch (state)
				{
					...
					case STATE_IDENTIFIER:
						switch (c)
						{
							...
							case '[':
							case '.':
								identifier = path.substring(anchor, i);
					...
					case STATE_OPERATOR:
						...
						{
							case '[':
								if (!current[identifier]) current[identifier] = [];
					...
					case STATE_INDEX:
						...
						{
							case ']':
								identifier = parseInt(path.substring(anchor, i));
								state = STATE_OPERATOR;
								++i;
								break;
							case '0':
			...
			if (Object.prototype.toString.apply(current) !== '[object Array]')
			{
				identifier = path.substring(anchor, path.length);
			}
			current[identifier] = values[path];
		}
	...
	{
		return JSON.parse(JSON.stringify(obj === void(0) ? null : obj));
	}
	...
	if (Array.find) throw "Not expected.";
	Array.find = function (arr, testCallback)
	{
		for (var length = arr.length, i = 0; i < length; ++i) if (testCallback(arr[i], i)) return i;
		return -1;
	}
