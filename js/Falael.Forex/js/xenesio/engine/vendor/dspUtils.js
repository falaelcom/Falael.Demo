"use strict";
//	view-source:https://www.arc.id.au/SpectrumAnalyser.html
//	https://www.arc.id.au/dspUtils-14.js
/*=========================================================
  Filename: dspUtils-14.js
  ...
  29Jun14 Used function wrapper to create namespace     ARC
  09Jun18 Added unfoldSpectrum                          ARC
  13Jun18 Added scaling by tSize to overlapsave filter  ARC
  18Jun18 bugfix: overlapsave going past ip array end
          overlapSaveFilter returns op arrray length    ARC
  =========================================================*/

var fft, unfoldSpectrum, calcFilter, kbWnd, hannWnd, convFilter, overlapSaveFilter, freqTranslate;
...
(function ()
{
	...
	unfoldSpectrum = function (Npair, ar, ai, opReal, opImag)
	{
		/*
		 ...
		 * or to view both -ve and +ve frequencies (if not frequency translated)
		 * Data is unfolded as follows:
		 ...
		 * Npair must be full size of FFT (a power of 2)
		*/
		var i, iPtr;

		for (i = 0, iPtr = Npair / 2 + 1; iPtr < Npair; i++ , iPtr++)
		{
			opReal[i] = ar[iPtr];
			opImag[i] = ai[iPtr];
		}
		for (iPtr = 0; i < Npair; i++ , iPtr++)
		{
			opReal[i] = ar[iPtr];
			opImag[i] = ai[iPtr];
		}
	...
	convFilter = function (H, ipAry, opAry, nPts, subSmpl)
	{
		/*
		 ...
		 * ip[] holds input data (length > subSmpl*nPts + M )
		 * op[] is output buffer
		...
		if (ipAry.length < subSmp * nPts + M)
		{
			console.error("insufficient input points for convFilter");
			return;
		}
		for (ip = Np, op = 0; op < nPts; ip += subSmp, op++)
		{
			for (i = 0; i < M; i++)
			{
				sum += H[i] * ipAry[ip];
	...
	overlapSaveFilter = function (H, ipReal, ipImag, opReal, opImag)
	{
		/*
		 ...
		 * length of the output data is returned
		 */

		var nPts = ipReal.length,
			M = H.length,
			Np = (M - 1) / 2,
			tSize = 1024,       // tSize is the FFT size to be used
			slab = tSize - M + 1,
			numPasses = Math.floor(nPts / slab),
			max,
			i, j, tr, ti,
			ar = [],
			ai = [],
		...
		max = 1; // Math.abs(H[Np]);   // H[Np] is center point of filter impulse response
		br[0] = H[Np] / max;
		for (j = 1; j <= Np; j++)
		{
			br[j] = H[Np + j] / max;
		...
		for (j = 0; j < numPasses; j++)
		{
			...
			for (i = 0; i < slab; i++)
			{
				...
				opReal[oPtr + i] = tSize * ar[Np + i];  // normalise for transform size
				opImag[oPtr + i] = tSize * ai[Np + i];
			}
	...
	freqTranslate = function (xReal, yReal, yImag, Npts, Fsmp, Fc)
	{
		/*
