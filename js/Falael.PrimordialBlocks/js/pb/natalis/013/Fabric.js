...
const Cardinality = Enum("Cardinality",
{
	Zero: Symbol("Zero"),
	AlephNull: Symbol("AlephNull"),
	AlephOne: Symbol("AlephOne"),
});

module.exports =

//	Class: Abstracts the means to distinguish between void (`Unset`), atoms, countable composites and uncountable composites based on `Native.Type`.
//	Remarks:
...
//		The `Fabric` class belongs to the `Transcribe` family of classes, which defines basic facilities for manipulation of ordered directed multigraphs.
...
class Fabric
{
	static the = new Fabric();

	//	Remarks: 
	getVertexCardinality(vertex)
	{
		return Array.isArray(vertex) ? Cardinality.AlephNull : Type.getType(vertex) === Type.Object ? Cardinality.AlephOne : Cardinality.Zero;
	}
	...
	//	Function: Performs a depth-first pre-order traversal of a graph; visits in order the graph root and all graph arcs that are present in the form of array elements, string properties and
	//		symbol properties; implements the Stalker Protocol.
	//	Parameter: `visit(state: any, deltaOut: integer, head: any, headCardinality: Cardinality, label: string | symbol | integer, labelCardinality: Cardinality): boolean` - called on root vertex or arc encounter;
	...
	//		- Parameter: `label: string | symbol | integer | void 0` - the label of the arc being visited or `void 0` with root and unroot;
	...
	//		Symbol properties are visited in uncountable graphs.
	walk(visit, visitorState, root)
	{
		...
		if (PB_DEBUG)
		{
			if (!Type.isBoolean(outcome)) throw new ReturnValueException(0x7D5B7E, "visit", outcome);
		}
		if (outcome)
		{
			...
			{
				for (const key in root) _traverse(root[key], key, Cardinality.AlephOne);
				for (let coll = Object.getOwnPropertySymbols(root), length = coll.length, i = 0; i < length; ++i) _traverse(root[coll[i]], coll[i], Cardinality.AlephOne);
			}
		}
		--pathLength;
		const outcome2 = visit(visitorState, visitedPathLength - pathLength);
		if (!outcome2) throw new ReturnValueException(0x67B2AB, "visit", outcome2);
		return visitorState;
		...
		function _traverse(head, label, labelCardinality)
		{
			...
			if (PB_DEBUG)
			{
				if (!Type.isBoolean(outcome)) throw new ReturnValueException(0x95A6A3, "visit", outcome);
			}
			visitedPathLength = pathLength;
			...
			{
				for (const key in head) _traverse(head[key], key, Cardinality.AlephOne);
				for (let coll = Object.getOwnPropertySymbols(head), length = coll.length, i = 0; i < length; ++i) _traverse(head[coll[i]], coll[i], Cardinality.AlephOne);
			}
			--pathLength;
		}
	}
...
module.exports.Fabric = module.exports;
